...
def make_translation_matrix(t_vectors):
    eye = np.eye(3)
    zeros = np.array([[0,0,0,0]])
    matrices = []
    for vector in t_vectors:
        top_part = np.concatenate((eye,vector.reshape(3,1)),1)
        matrices.append(np.concatenate((top_part,zeros),0))
    return matrices
